...
DevelopVersionLineNumber=`grep -nE 'version.develop.*=' A.podspec | cut -d : -f1`
sed -i "" "${DevelopVersionLineNumber}s/${DevelopVersionNumber}/${NewDevelopVersionNumber}/g" A.podspec
...
TestVersionLineNumber=`grep -nE 'version.test.*=' A.podspec | cut -d : -f1`
sed -i "" "${TestVersionLineNumber}s/${TestVersionNumber}/${NewTestVersionNumber}/g" A.podspec
echo "current test version is ${TestVersionNumber}, new version is ${NewTestVersionNumber}, line number is ${TestVersionLineNumber}"

GrayVersionLineNumber=`grep -nE 'version.gray.*=' A.podspec | cut -d : -f1`
sed -i "" "${GrayVersionLineNumber}s/${GrayVersionNumber}/${NewGrayVersionNumber}/g" A.podspec
...
VersionLineNumber=`grep -nE 's.version.*=' A.podspec | cut -d : -f1`
sed -i "" "${VersionLineNumber}s/${VersionNumber}/${NewVersionNumber}/g" A.podspec
...
pod repo push ${Repo} A.podspec --verbose --allow-warnings --use-libraries --use-modular-headers
